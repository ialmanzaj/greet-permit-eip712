...
contract Greeter {
    ...
    function hash(Greeting memory greeting) internal pure returns (bytes32) {
        return keccak256(
            abi.encode(
                keccak256("Greeting(string text,uint deadline)"), keccak256(bytes(greeting.text)), greeting.deadline
            )
        );
    }
    ...
    function verify(
        ...
        returns (bool)
    {
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, hash(greeting)));
